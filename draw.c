...
void draw_environment(World world, Rotate* rotate, Move move ){
   ...
   glPopMatrix();
    ...
    glPushMatrix();
    		...
    		glScalef(1.0f, 1.0f, 1.0f);
    		glRotatef(180, 0, 0, 1);
			glRotatef(rotate->mars_rotation, 0, 1, 1);
   ...
   glPopMatrix();
    ...
    glPushMatrix();
        ...
        glTranslatef(move.asteroid.x,move.asteroid.y,move.asteroid.z);
     	glMaterialfv(GL_FRONT, GL_AMBIENT, world.asteroid.material_ambient);
        glBindTexture(GL_TEXTURE_2D, world.asteroid.texture);
    	glScalef(1.0f, 1.0f, 1.0f);
        glRotatef(90, 1, 0, 0);
        glRotatef(270, 0, 1, 0);
		glRotatef(rotate->asteroid_rotation, 1, 1, 1);
        draw_model(&world.asteroid.model);
